...
SELECT c.nome, sum(p.valor) as soma_do_valor, count(p.id)
FROM clientes c
...
SELECT c.nome as categoria, p.nome as produto, avg(v.preco) as media_preço_venda, sum(v.unidades) as total_unidades_vendidas
FROM categorias c 
